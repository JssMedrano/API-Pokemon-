...
else
{
    ...
    var sqlitePath = Path.Combine(dataDir, "entrenadores.db");

    builder.Services.AddDbContext<ApplicationDbContext>(option =>
        option.UseSqlite($"Data Source={sqlitePath}"));
}
...
// Porta
var url = Environment.GetEnvironmentVariable("ENTRENADORES_API_URL");
